...
export const seedDataFromXlsx = async () => {
	let coordinatesIndex, boardNameIndex, employeeNameIndex, emailindex, unityNameIndex, positionName, bodyEmployee, bodyBoard, bodyUnity, bodySale;
	...
	for (let i of formatedArray) {
		for (let j in i) {
			if (i[j].find((item: string) => item === 'Diretor Geral')) {
				...
				bodyEmployee = { email: i[j][emailindex], name: i[j][employeeNameIndex], positionName };
				...
				switch (positionName) {
					case "Diretor":
						...
						break;
					case "Gerente":
						bodyUnity = { name: i[j][unityNameIndex], managerId: id, boardName: i[j][boardNameIndex], coordinates: i[j][coordinatesIndex] };
...
const seedEmployee = async (content: ISeedEmployee) => {
	...
	if (!employee) {
		const { id } = await prisma.positions.findUnique({ where: { name: positionName } });
...
const seedSales = async (content: ISeedSales) => {
	...
	const body = { employeeId, unityId: unity.id, coordinates: unity.coordinates, value: 100, saleDate: new Date() };
